...
let has_schedule_conflict new_course my_courses =
  List.exists
    (fun existing_course ->
      is_conflict (get_course_schedule existing_course) (get_course_schedule new_course)
    )
    my_courses
...
let add_course_ID netid course_id =
  try
    let course_to_add = 
      List.find (fun c -> (get_course_id c) = course_id) cs_courses in
    let user = List.find (fun u -> (get_netid u) = netid) users in
    let current_credits = total_credits !my_courses in

    if current_credits +. (get_course_credits course_to_add) >
...
let add_course_name netid course_name =
  try
    let course_to_add =
      List.find
        (fun c ->
          String.lowercase_ascii (get_course_name c) = String.lowercase_ascii course_name)
        cs_courses
    in
    let user = List.find (fun u -> (Users.get_netid u) = netid) Users.users in
    let current_credits = total_credits !my_courses in

    if current_credits +. (get_course_credits course_to_add) > get_credit_limit (Users.get_college user)
    then print_endline "Cannot add course: credit limit exceeded."
    else if List.exists (fun c -> (get_course_name c) = course_name) !my_courses then
      print_endline "You are already enrolled in this course."
    else if has_schedule_conflict course_to_add !my_courses then
      print_endline
...
let drop_course_ID netid course_id =
  try
    let course_to_drop = List.find (fun c -> get_course_id c = course_id) !my_courses in
    my_courses := List.filter (fun c -> get_course_id c <> course_id) !my_courses;
    let user = List.find (fun u -> Users.get_netid u = netid) Users.users in
    Users.set_total_credits ((Users.get_total_credits user) -. get_course_credits course_to_drop) user;
...
let drop_course_name netid course_name =
  try
    ...
    in
    my_courses :=
      List.filter (fun c -> (get_course_name c) <> get_course_name course_to_drop) !my_courses;
    let user = List.find (fun u -> (Users.get_netid u) = netid) Users.users in
    Users.set_total_credits ((Users.get_total_credits user) -. get_course_credits course_to_drop) user; 
    print_endline ("Dropped course: " ^ get_course_name course_to_drop)
  with Not_found ->
